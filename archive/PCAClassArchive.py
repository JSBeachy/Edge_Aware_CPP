...
class PCABounding:
    ...
    def PCA_Calculation(self):
        ...
        print(np.shape(centered_points))
...
class Best_Fit_CPP(PCABounding):
    ...
    def splitting(self, edge_list, hull_vertices_list):
            ...
            #dir1, dir2 are number of edge points between corners (including the corners)
            ...
            if dir1_len<=2:
                ...
                midpoint1=(index[1]+((index[0]-index[1])%len_my_list)//2)%len_my_list 
                midpoint2=(index[0]+((index[1]-index[0])%len_my_list)//2)%len_my_list 
    ...
    def fit_curve3d(self, points, Bezier_order=5, sample_points=20):
        # Define Bézier degree and location of 
        n = Bezier_order
        n_opt = sample_points
        ...
        for i in range(n+1):
            for j in range(n+1):
                Bpatch[i, j] = (-1)**(j-i) * math.comb(n, j) * math.comb(j, i)

        # Optimization variable for X, Y, and Z coordinates of Bézier Control Points
        Pix = cp.Variable(n+1)
        Piy = cp.Variable(n+1)
        Piz = cp.Variable(n+1)    
        ...
        bezier_points = np.array([self.bezier_curveN(n, t, Pi) for t in t_fine])

        return bezier_points

    def bezier_curveN(self, n, t, control_points):
        """
        Compute the Bézier curve at parameter t in 3D.
        
        Parameters:
        n (int): Degree of the Bézier curve.
        t (float): Parameter in range (0,1).
        control_points (np.ndarray): (n+1) x 3 array of control points.
        
        Returns:
        np.ndarray: (x, y, z) point on the Bézier curve.
        """
        if not (0 <= t <= 1):
